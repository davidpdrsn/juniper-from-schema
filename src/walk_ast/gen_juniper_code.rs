...
fn gen_type_def(type_def: TypeDefinition, out: &mut Output) {
    ...
    match type_def {
        ...
        Interface(interface_type) => gen_interface(interface_type, out),
...
fn gen_obj_type(obj_type: ObjectType, out: &mut Output) {
    ...
    let fields = field_tokens
        .into_iter()
        .map(|field| gen_field(field, &struct_name, &trait_name));
...
fn gen_field(field: FieldTokens, struct_name: &Ident, trait_name: &Ident) -> TokenStream {
...
fn gen_interface(interface: InterfaceType, out: &mut Output) {
    ...
    let field_token_streams = field_tokens
        ...
        .map(|field| {
            ...
            let arms = implementors.clone().into_iter().map(|variant| {
                ...
                let body = gen_field_body(field.clone(), quote! {inner}, &struct_name, &trait_name);
            ...
            quote! {
                #description
                field #field_name(#all_args) -> juniper::FieldResult<#field_type> {
                    match *self {
                        #(#arms),*
                    }
                }
